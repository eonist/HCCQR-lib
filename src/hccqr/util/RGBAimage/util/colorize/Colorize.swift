...
internal class Colorize {
   ...
   internal static func colorize(images:[Image], colorMap:ColorMap, moduleMultiplier:Int, scale:Int) -> Image? {
      ...
      guard let image:Image = RGBAImage.uiImage(rgbaImage: result, scale:CGFloat(scale)) else {Swift.print("Colorize.colorize() - Unable to convert to UIImage");return nil}
      return image
   }
}
