...
extension MemLeakTest {
   ...
   static func testFixingMemLeak() {
      let setup: HCCQRConfig = .init(qr: .init(qrVersion: .v10, ecLevel: .l), output: .init(scale: .init(6, 2)))
//      let config: QRConfig = (setup.qrVersion, .byte, setup.ecLevel) // Config
      (0..<40).forEach { _ in
         guard let data = HCCQRData.randomData(setup: setup) else { Swift.print("err data"); return }
         let img: Image? = try? Writer.image(data: data, config: setup, parallel: true)
