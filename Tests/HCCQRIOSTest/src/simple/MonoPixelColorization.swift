...
class MonoPixelColorization {
   ...
   static func testColorizingMonoPixel() -> Bool {
      guard let pixelA: Pixel = try? Colorizer.colorize(pixels: [false, true], colorMap: ColorMap.colorMap()) else { fatalError("err") }// -> RedPixel ⚠️️ complete this
      ...
      guard let pixelB: Pixel = try? Colorizer.colorize(pixels: [true, true], colorMap: ColorMap.colorMap()) else { fatalError("err") }// -> BluePixel
      ...
      guard let pixelC: Pixel = try? Colorizer.colorize(pixels: [true, false], colorMap: ColorMap.colorMap()) else { fatalError("err") }// -> BluePixel
