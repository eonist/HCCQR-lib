...
class MonoPixelColorization {
   ...
   static func testColorizingMonoPixel() -> Bool {
      guard let pixelA: Pixel = try? Colorizer.colorize(pixels: [false, true], pallete: ColorPallete.fourColors()) else { fatalError("err") }// -> RedPixel ⚠️️ complete this
      ...
      guard let pixelC: Pixel = try? Colorizer.colorize(pixels: [true, false], pallete: ColorPallete.fourColors()) else { fatalError("err") }// -> BluePixel
