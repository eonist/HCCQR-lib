...
final class MonoPixelColorization {
   ...
   static func testColorizingMonoPixel() -> Bool {
      guard let pixelA: Pixel = try? Colorizer.colorize(pixels: [true, false], pallete: .cp4()) else { fatalError("err") }// -> RedPixel ⚠️️ complete this
      Swift.print("pixelA:  \(pixelA)")
      let isPixelARed: Bool = Pixel.isMatching(a: pixelA, b: .red)
      Swift.print("isPixelARed:  \(isPixelARed)")
      guard let pixelB: Pixel = try? Colorizer.colorize(pixels: [true, true], pallete: .cp4()) else { fatalError("err") }// -> BluePixel
      Swift.print("pixelB:  \(pixelB)")
      let isPixelBBlue: Bool = Pixel.isMatching(a: pixelB, b: .blue)
      Swift.print("isPixelBBlue:  \(isPixelBBlue)")
      guard let pixelC: Pixel = try? Colorizer.colorize(pixels: [false, true], pallete: .cp4()) else { fatalError("err") }// -> BluePixel
