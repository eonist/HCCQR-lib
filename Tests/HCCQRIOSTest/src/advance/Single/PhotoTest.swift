...
extension PhotoTest {
   ...
   internal static func test() -> Bool {
      ...
      guard let data: Data = try? Reader.data(rgbaRep: rgbaRep, scheme: .scheme(scheme: CType.c4.cs, darkMode: true), parallel: true).qrData else { Swift.print("err"); return false }// extract data from the hccqrImg
