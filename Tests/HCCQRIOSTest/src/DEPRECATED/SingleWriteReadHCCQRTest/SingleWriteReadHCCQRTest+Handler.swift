...
extension SingleWriteReadHCCQRTest {
   ...
   static func onWriteComplete(rgbaImage: RGBARep, randomData: Data, onComplete: @escaping OnComplete) {
      ...
      Reader.data(rgbaRep: rgbaImage) { result in // Start reading the hccqr
         guard let value: Reader.ReadPayload2 = result.value() else { Swift.print("🚫 err:  \(result.errorStr)"); return }
    ...
    */
   private static func onReadComplete(dataAndImages: Reader.ReadPayload2, randomData: Data, onComplete: OnComplete) {
