...
//            Swift.print("qrCode:  \(qrCode)")//testing
   ...
   func createQRImageView(){
      ...
      guard let qrImage:UIImage = QRImageUtil.qrImage(str: string, size: .init(width:side,height:side), ecLevel: .l) else {Swift.print("unable to create UIImage");return }
      ...
      for (i,_) in arr.enumerated() {
         if (i % 12) == 0 {
            let versionString:String = {
               let modeStrings:[String] = (i..<(i+12)).compactMap { e in
                  if (e % 4) == 0 {
                     let modeString:String = "l:\(arr[e+0]),m:\(arr[e+1]),q:\(arr[e+2]),h:\(arr[e+3])"
                     return modeString
                  }
                  return nil
               }
