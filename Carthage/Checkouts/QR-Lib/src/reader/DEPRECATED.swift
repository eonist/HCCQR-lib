...
extension QRWriter {
    ...
    * - Fixme: ⚠️️ Move this out of this lib and into app libs using this lib 👌
    */
   #if os(macOS)
   public static func imageView(nsImage: Image, rect: CGRect) -> NSImageView {
      let imageView: NSImageView = .init(frame: rect)
...
extension QRWriter {
    ...
    * - Fixme: ⚠️️ this is different for mac. see deprecated code, i think it's the same now
...
extension QRReader {
    ...
    * ## Examples:
    * qrCode(image: image)
    * - Fixme: ⚠️️ rename to string(image)
    */
    ...
    * - Fixme: ⚠️️ check if topLeft is the same as bounds.topleft, if not you have a more use-full rectangle outline
    ...
    * - Fixme: ⚠️️ rename to stringAndFrame?
    */
...
extension QRReader {
    ...
    * - Note: iOS provides descriptor in the cameraCapture call
    * ## Examples:
    * qrCode(descriptor: descriptor)//Data()
    * - Fixme: ⚠️️ rename to data(image)
    */
