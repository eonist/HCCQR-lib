...
extension RGBARepUtil {
    ...
    * - Note: Seems like there isnt much benefit of this method, vs the existing rgbaRep function
    */
   static func rgbaRepresentation(image: Image) throws -> RGBARep {
      if let cgImg = image.cgImage() {
         let rect = BufferRect(0, 0, Int(cgImg.width), Int(cgImg.height))
         return try rgbaRepresentation(rect: rect) { context in
            context.draw(cgImg, in: rect.cgRect)
         }
      } else if let ciImg: CIImage = image.ciImg() {
         let rect = BufferRect(0, 0, Int(ciImg.extent.width), Int(ciImg.extent.height))
         return try rgbaRepresentation(rect: rect) { context in
            let ciContext = CIContext(cgContext: context, options: nil)
            ciContext.draw(ciImg, in: rect.cgRect, from: rect.cgRect)
         }
      }
    ...
    */
   private static func rgbaRepresentation(rect: BufferRect, draw: DrawContext) throws -> RGBARep {
