...
internal class PixelDataUtil {
    ...
    */
   internal static func rgba(uiColor: Color) throws -> PixelData.RGBA {
      ...
      #if os(iOS)
      guard uiColor.getRed(&fRed, green: &fGreen, blue: &fBlue, alpha: &fAlpha) else { throw NSError.init(domain: "Could not extract RGBA components", code: 0) }
      #elseif os(macOS)
      guard let ciColor = CIColor(color: uiColor) else { throw NSError.init(domain: "PixelDataUtil.rgba() - Could not convert nsColor to CIColor", code: 0) }
      fRed = ciColor.red // 1.0
      fGreen = ciColor.green // 0.0
      fBlue = ciColor.blue // 0.0
      fAlpha = ciColor.alpha // 1.0 or use nsColor.alphaComponent
      #else
      throw NSError.init(domain: "os not supported", code: 0)
      #endif
      // Fixme: ⚠️️ Group the bellow into one line
