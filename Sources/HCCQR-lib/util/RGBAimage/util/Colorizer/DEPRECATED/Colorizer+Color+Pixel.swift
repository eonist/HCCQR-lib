...
extension Colorizer {
   ...
   /**
    * Converts multiple layers of b&w pixels into one color color-pixel (on the basis of a colorMap rule set)
    * 
    ...
    * - Fixme: ⚠️️⚠️️⚠️️ This method will probably be deprecated, because we use the grayscale version of it now ⚠️️⚠️️⚠️️
    */
   static func colorize(pixels: [PixelData], colorMap: ColorMap) throws -> PixelData {
      let findColor: MatchColor = { colorMapItem in
         if colorMapItem.idx.count != pixels.count { throw NSError(domain: "Colorize.colorize - colorMap does not match pixel layer count", code: 0) } // just a quick check, basically calling .count doesn't cost anything
