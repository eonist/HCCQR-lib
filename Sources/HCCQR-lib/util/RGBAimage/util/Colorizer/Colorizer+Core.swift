...
extension Colorizer {
   ...
   private static func colorize(pixels: [PixelData], colorMap: ColorMap) throws -> PixelData {
      let findColor: (ColorMapItem) throws -> Bool = { colorMapItem in
         if colorMapItem.idx.count != pixels.count { throw NSError.init(domain: "Colorize.colorize - colorMap does not match pixel layer count", code: 0) }
      ...
      guard let color: Color = try colorMap.first(where: findColor)?.color else { throw NSError.init(domain: "Unable to colorize", code: 0)}
