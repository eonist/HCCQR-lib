...
extension AlignMarkUtil{
    ...
    * Debug marks
    */
   static func drawDebugSquare(metric:AlignBoxMetric, pos:CGPoint) -> (CAShapeLayer,CAShapeLayer){
      let rect:CGRect = .init(x:pos.x,y:pos.y,width:metric.squareLength,height:metric.squareLength)
      ...
      let innerShape:CAShapeLayer = {
         ...
         let blackShape = CGShapeUtil.drawRect(shapeLayer:.init(), rect:innerRect, style:(UIColor.red.withAlphaComponent(0.5),nil,nil))
         return blackShape
      }()
